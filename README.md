...
- **生效来源透明**：`GET /llm/usage` 返回 `pricing: {source, price}`，前端实时标注当前计价来源——「✏️ 自定义 / 内置定价表 / 本地模型（0 元）/ ⚠️ 未知模型按保守默认价估算」，让估算偏差可见。
- **前端设置面板**：「💰 用量与预算」区块新增「自定义单价」三个输入框（输入 / 缓存命中 / 输出），保存走 `POST /llm/config` 整包提交（保留其他模型已配单价，只增删当前模型这一条）；清空输入保存即恢复内置计价。切供应商 / 模型时输入框自动切换到对应模型的自定义值。
- 新增 `tests/test_llm_budget.py` 6 项（自定义覆盖内置、裸模型名、二元组写法、非法回退、来源标注、summary 透出）；**全量 pytest 77 项全绿**。
